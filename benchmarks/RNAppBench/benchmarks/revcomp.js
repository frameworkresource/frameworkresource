...
async function startRevComp(fileName) {
  const ret = await fs.readFileAssets('files/'+fileName+'.txt', "utf8");
  await revcomp(ret);
  return 'finished';
}

async function revcomp(data) {
  ...
  async function mainThread(data) {
    ...
    function reverseCompPrint(line) {
      ...
      metaI = _metaI + count;
      numLines = _numLines;
      //output += decoder.decode(target.slice(0, count))
      output += target.slice(0, count).toString('ascii')
    }
      ...
      while (chunk) {
        while (true) {
          ...
          if (needHeader) {
            ...
            output += headerPartial.toString('ascii') + chunk.slice(0, headerEnds).toString('ascii');
            
            //const headerEnds = chunk.indexOf(10);
            //output += headerPartial.toString('ascii') + decoder.decode(chunk.slice(0, headerEnds));
            headerPartial = '';
          ...
          if (caretIdx > -1) {
            ...
            if (headerEnds < 0) {
              ...
              // log the header
              output += chunk.slice(0, headerEnds).toString('ascii');
              //output += decoder.decode(chunk.slice(0, headerEnds));
