...
// 13/01/2023
// https://benchmarksgame-team.pages.debian.net/benchmarksgame/program/spectralnorm-dartexe-6.html
...
Future<void> runSpectralParallel(int nProcessors, StringSink writer) async {
  final mainIsolate = ReceivePort();
  final n = 19500;
  nIsolates = nProcessors;
  ...
  await mainIsolate.listen((dynamic s) {
    if (s is Span) {
      ...
      if (--awaited == 0) { // Wait until all nIsolates are known.
        var res = spectralNorm(n);
        res.then((value) {
        writer.write(value.toStringAsFixed(9)); 
        print(value.toStringAsFixed(9));
        mainIsolate.close();
        });
      }
    }
