...
// 21/05/2022
// https://benchmarksgame-team.pages.debian.net/benchmarksgame/program/knucleotide-dartexe-3.html
...
void sort(CodeList codes, int length, StringSink writer) {
...
Future runKnucleotide(Stream<String> stream, StringSink writer) async {
  ...
  sort(sequence, 1, writer);
  sort(sequence, 2, writer);
  (await a).forEach(writer.writeln);
  (await b).forEach(writer.writeln);
  (await c).forEach(writer.writeln);
}
